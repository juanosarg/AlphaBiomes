            ...
            if (ModLister.HasActiveModWithName("Alpha Animals")) {
                ...
                if (totalCount < 25)
                {
                    ...
                    if (pawnkind != null)
                    {
                        ...
                        if (pawnToCreate.Spawned && this.parent.Map != null && Faction.OfInsects!=null)
                        {
                            Lord pawnLord = null;
                            if (pawnToCreate.Map.mapPawns.SpawnedPawnsInFaction(Faction.OfInsects).Any((Pawn p) => p != pawnToCreate))
                            {
                                Pawn closestAlly = (Pawn)GenClosest.ClosestThing_Global(pawnToCreate.Position, pawnToCreate.Map.mapPawns.SpawnedPawnsInFaction(Faction.OfInsects), 30, (Thing p) => p != pawnToCreate && ((Pawn)p).GetLord() != null);
                                if (closestAlly != null) {
                                    pawnLord = closestAlly.GetLord();
                                }
                                
                            }
                            if (pawnLord == null || !pawnLord.CanAddPawn(pawnToCreate))
                            {
                                pawnLord = LordMaker.MakeNewLord(Faction.OfInsects, new LordJob_DefendPoint(pawnToCreate.Position,10,10), pawnToCreate.Map);
                            }
                            if (pawnLord?.LordJob.CanAutoAddPawns ?? false)
                            {
                                pawnLord.AddPawn(pawnToCreate);
                            }
                        }
